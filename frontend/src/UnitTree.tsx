...
const SIDE_ORDER: Array<{ side: Side; label: string }> = [
  { side: 'allied', label: '盟军' }, { side: 'soviet', label: '苏军' }, { side: 'yuri', label: '尤里' }, { side: 'neutral', label: '中立与通用' },
]
...
export function FallbackTypeIcon({ category, size = 15 }: { category: string; size?: number }) {
  ...
  const Icon = type === '步兵' ? Users : type === '载具' ? Truck : type === '飞机' ? Plane : type === '建筑' ? Building2 : type === '超级武器' ? Sparkles : type === '国家' ? Flag : type === '武器' ? Crosshair : type === '弹头' ? Bomb : type === '弹体' ? Rocket : Box
...
function parseDirectReferences(raw: string, rows: UnitTreeRow[]) {
  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    if (!source || !BUILDABLE_TYPES.has(normalizedType(source.category))) continue
    const key = match[1].trim()
    if (!DIRECT_REFERENCE_KEYS.has(key.toLowerCase())) continue
    for (const token of match[2].split(';', 1)[0].split(',').map(v => v.trim()).filter(Boolean)) {
      const target = rowById.get(token.toLowerCase())
      if (!target || target.id.toLowerCase() === source.id.toLowerCase()) continue
      const list = outgoing.get(source.id.toLowerCase()) ?? []
      if (!list.some(edge => edge.row.id.toLowerCase() === target.id.toLowerCase() && edge.key.toLowerCase() === key.toLowerCase())) list.push({ row: target, key })
      outgoing.set(source.id.toLowerCase(), list)
    }
  }
  return outgoing
}
...
function relationLabel(key: string) {
  const labels: Record<string, string> = { primary: '主武器', secondary: '副武器', eliteprimary: '精英主武器', elitesecondary: '精英副武器', occupyweapon: '驻军武器', eliteoccupyweapon: '精英驻军武器', deathweapon: '死亡武器', deploysinto: '部署建筑', undeploysinto: '反部署', spawns: '生成单位', enslaves: '奴隶单位' }
...
export function UnitTree({ rows, selectedId, query, documentEpoch, onSelect }: Props) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (!editor || !workspace) return setDockRect(null)
      const e = editor.getBoundingClientRect(), w = workspace.getBoundingClientRect()
      setDockRect({ left: e.left, width: Math.max(0, w.right - e.left), bottom: Math.max(0, innerHeight - w.bottom), tableVisible: Boolean(editor.querySelector('.parameterTablePane')) })
    }
    update()
    const workspace = document.querySelector<HTMLElement>('.workspace')
    const ro = typeof ResizeObserver !== 'undefined' && workspace ? new ResizeObserver(update) : null
  ...
  const directReferences = useMemo(() => parseDirectReferences(rawRules, rows), [rawRules, rows])
  ...
  const branches = useMemo(() => selectedRow ? directReferences.get(selectedRow.id.toLowerCase()) ?? [] : [], [directReferences, selectedRow])
  const hasReferenceChain = branches.length > 0

  useEffect(() => { document.body.classList.toggle('has-reference-chain', hasReferenceChain && Boolean(dockRect?.tableVisible)); return () => document.body.classList.remove('has-reference-chain') }, [dockRect?.tableVisible, hasReferenceChain])
  ...
  const sideGroups = useMemo<SideGroup[]>(() => SIDE_ORDER.map(meta => ({ ...meta, types: UNIT_TYPE_ORDER.map(name => ({ name, units: filteredRows.filter(row => !WEAPON_TYPES.has(normalizedType(row.category)) && normalizedType(row.category) === name && navigation.sideOf(row.id) === meta.side) })).filter(group => group.units.length) })).filter(group => group.types.length), [filteredRows, navigation])
  ...
  const isOpen = (key: string, defaultOpen = true) => searching || (key in expanded ? expanded[key] : defaultOpen)
  ...
  const expandFor = (row: UnitTreeRow) => {
    ...
    if (WEAPON_TYPES.has(type)) setExpanded(value => ({ ...value, weapons: true, [`weapon:${type}`]: true }))
    else { const side = navigation.sideOf(row.id); setExpanded(value => ({ ...value, [`side:${side}`]: true, [`side:${side}:${type}`]: true })) }
  }
  const selectReference = (row: UnitTreeRow) => { expandFor(row); onSelect(row) }

  useEffect(() => {
    if (!selectedId) return
    const type = rows.find(row => row.id.toLowerCase() === selectedId.toLowerCase())
    if (type) expandFor(type)
  ...
  return <div className="unitHierarchy">
    ...
    <div className="unitTreeScroller">
      {sideGroups.map(group => { const sideKey = `side:${group.side}`, sideOpen = isOpen(sideKey); return <section className="unitSideGroup" key={group.side}><button className={`unitTreeLevel sideLevel side-${group.side}`} onClick={() => toggle(sideKey, sideOpen)}>{sideOpen ? <ChevronDown size={15}/> : <ChevronRight size={15}/>}<strong>{group.label}</strong><em>{group.types.reduce((n, x) => n + x.units.length, 0)}</em></button>{sideOpen && <div className="unitSideChildren">{group.types.map(type => { const typeKey = `${sideKey}:${type.name}`, open = isOpen(typeKey); return <section className="unitTypeGroup" key={type.name}><button className="unitTreeLevel legacyType" onClick={() => toggle(typeKey, open)}>{open ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}<FallbackTypeIcon category={type.name} size={14}/><strong>{type.name}</strong><em>{type.units.length}</em></button>{open && <div className="unitLeaves legacyLeaves">{type.units.map(unit => <UnitLeaf key={unit.id} unit={unit} exclusiveCountry={exclusiveCountryOf(unit)} selectedId={selectedId} onSelect={onSelect}/>)}</div>}</section>})}</div>}</section> })}
      {weaponGroups.length > 0 && (() => { const key = 'weapons', open = isOpen(key); return <section className="unitSideGroup weaponRoot"><button className="unitTreeLevel sideLevel weaponLevel" onClick={() => toggle(key, open)}>{open ? <ChevronDown size={15}/> : <ChevronRight size={15}/>}<Crosshair size={15}/><strong>武器类</strong><em>{weaponGroups.reduce((n, x) => n + x.units.length, 0)}</em></button>{open && <div className="unitSideChildren">{weaponGroups.map(type => { const typeKey = `weapon:${type.name}`, typeOpen = isOpen(typeKey); return <section className="unitTypeGroup" key={type.name}><button className="unitTreeLevel legacyType" onClick={() => toggle(typeKey, typeOpen)}>{typeOpen ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}<FallbackTypeIcon category={type.name} size={14}/><strong>{type.name}</strong><em>{type.units.length}</em></button>{typeOpen && <div className="unitLeaves legacyLeaves">{type.units.map(unit => <UnitLeaf key={unit.id} unit={unit} selectedId={selectedId} onSelect={onSelect}/>)}</div>}</section>})}</div>}</section> })()}
    </div>

    {hasReferenceChain && selectedRow && dockRect?.tableVisible && <div className="referenceChainDock referenceBranchDock" style={{ left: dockRect.left, width: dockRect.width, bottom: dockRect.bottom }}><span className="referenceChainLabel">引用链</span><div className="referenceBranchRoot"><div className="referenceChainCurrent"><UnitIcon unit={selectedRow} compact/><span><b>{selectedRow.label}</b><small>{selectedRow.id}</small></span></div><div className="referenceBranchStem"/></div><div className={`referenceBranchList ${branches.length > 1 ? 'branched' : ''}`}>{branches.map(edge => <div className="referenceBranch" key={`${edge.key}:${edge.row.id}`}><span className="referenceBranchLine"/><ReferenceNode edge={edge} onSelect={selectReference}/></div>)}</div></div>}
  </div>
}
