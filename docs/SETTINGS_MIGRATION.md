...
便携版使用主程序旁边的 `resources` 目录；开发模式使用 `src/rulesmd_editor/resources`。前端仍会把少量值镜像到 WebView localStorage，供现有同步初始化逻辑读取，但 `app-config.json` 是跨启动保存的资源文件。
...
参数表“参数名”列可直接编辑：Enter 或失焦保存，Esc 放弃。清空后保存会移除该 Key 的用户覆盖并恢复内置描述。
