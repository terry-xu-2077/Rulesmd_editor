...
export function AddUnitDialog({ open, rows, initialCategory, onClose, onCreated }: Props) {
  ...
  const editableOptions = useMemo(() => (templateData?.options ?? []).filter(option => !['name', 'uiname'].includes(option.key.toLowerCase())), [templateData])
  const visibleOptions = useMemo(() => {
    const q = parameterQuery.trim().toLowerCase()
    if (!q) return editableOptions
    return editableOptions.filter(option => `${option.key} ${option.label} ${drafts[option.key] ?? option.value}`.toLowerCase().includes(q))
  }, [drafts, editableOptions, parameterQuery])
  ...
  useEffect(() => {
    ...
    void workspaceApi.section(templateId).then(data => {
      ...
      setDrafts(Object.fromEntries(data.options.map(option => [option.key, option.value])))
    }).catch(err => {
  ...
  async function createUnit() {
    ...
    try {
      const result = await workspaceApi.createUnit({
        ...
        values: templateData.options.map(option => ({ key: option.key, value: drafts[option.key] ?? option.value })),
      })
  ...
  return <Dialog open={open} title="添加新单位" icon={<PackagePlus size={18}/>} size="wide" onClose={onClose}>
    <div className="addUnitDialog">
      ...
      <div className="addUnitRegistrationHint">
        ...
        <span>创建时会自动写入 [{CATEGORY_ROOT[category] || 'Types'}] 的下一个数字 ID；UIName 会自动改为 Name:{sectionName.trim() || 'NEWUNIT'}，Name 使用上方必填注释。</span>
      </div>

      <div className="addUnitParameterToolbar">
        <div><strong>模板参数</strong><span>{selectedTemplate ? `${selectedTemplate.label} [${selectedTemplate.id}]` : '未选择模板'}</span></div>
      ...
      <div className="addUnitParameterTable">
        <div className="addUnitParameterHeader"><span>参数名</span><span>Key</span><span>初始值</span></div>
        <div className="addUnitParameterRows">
          {loading && <div className="addUnitEmpty">正在读取模板参数…</div>}
          {!loading && visibleOptions.map(option => <div className="addUnitParameterRow" key={option.line_id}>
            <strong>{option.label || option.key}</strong>
            <code>{option.key}</code>
            <TextField value={drafts[option.key] ?? option.value} onChange={value => setDrafts(current => ({ ...current, [option.key]: value }))}/>
          </div>)}
      ...
      <footer className="addUnitActions">
        ...
        <Button variant="accent" disabled={creating || loading || !templateId || !validSectionName(sectionName) || !comment.trim()} onClick={() => void createUnit()}><PackagePlus size={16}/>{creating ? '正在创建…' : '创建单位'}</Button>
      </footer>
    </div>
  </Dialog>
}
