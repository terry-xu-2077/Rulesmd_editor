...
import './visual-icons.css'

/**
 * Unified visual icon system.
 *
 * Three sources are intentionally kept behind one API:
 * 1. tile: coordinate-based sprite/tile sheets used by legacy RA2 assets;
 * 2. open: open-source Lucide icons for semantic/generated entries;
 * 3. simple: small element + text cards for entries without artwork.
 */
...
const OPEN_ICON_COMPONENTS: Record<OpenIconKind, LucideIcon> = {
  generic: Box,
  technology: Sparkles,
  barracks: Users,
  power: Zap,
...
const SEMANTIC_VALUE_ICON: Record<string, OpenIconKind> = {
  TECH: 'technology',
...
/** Type 1: coordinate-based tile/sprite icon. */
...
/** Type 2: open-source Lucide icon. */
export function createOpenIcon(kind: OpenIconKind, size = 15, className = ''): ReactNode {
  const Icon = OPEN_ICON_COMPONENTS[kind] ?? OPEN_ICON_COMPONENTS.generic
  return createElement(
    'span',
    { className: `rulesGeneratedOpenIcon ${className}`.trim() },
    createElement(Icon, { size, 'aria-hidden': true }),
  )
}

/** Type 3: compact element + text icon/card. */
export function createSimpleIcon(text: string, options: { kind?: OpenIconKind; withElement?: boolean; className?: string } = {}): ReactNode {
  const clean = text.trim() || '?'
  const kind = options.kind ?? 'generic'
  const children: ReactNode[] = []
  if (options.withElement) children.push(createOpenIcon(kind, 11, 'rulesSimpleIconElement'))
  children.push(createElement('span', { key: 'text' }, clean))
  return createElement('span', { className: `rulesGeneratedSimpleIcon ${options.className || ''}`.trim(), title: clean }, ...children)
}

export function semanticOpenIconKind(value: string): OpenIconKind | undefined {
...
export function resolveVisualIcon(value: string, options: VisualIconResolverOptions = {}): ReactNode | undefined {
  ...
  if (options.countryFallback) return createSimpleIcon(options.label || value, { kind: 'side', withElement: false, className: 'countryFallback' })
  return undefined
}

function installOptionResolver() {
...
const COUNTRY_FALLBACK_ATTRIBUTE = 'data-country-fallback-label'
let bridgeScheduled = false

function syncCountryFallbackCards() {
  bridgeScheduled = false
  if (typeof document === 'undefined') return

  const leaves = document.querySelectorAll<HTMLElement>('.unitTreeLeaf')
  leaves.forEach(leaf => {
    const isCountryWithoutTile = Boolean(leaf.querySelector('.unitTreeIcon.fallback.semantic svg.lucide-flag'))
    if (!isCountryWithoutTile) {
      leaf.removeAttribute(COUNTRY_FALLBACK_ATTRIBUTE)
      return
    }
    const label = leaf.querySelector<HTMLElement>('.unitTreeLeafText b')?.textContent?.trim()
    if (label) leaf.setAttribute(COUNTRY_FALLBACK_ATTRIBUTE, label)
  })

  const header = document.querySelector<HTMLElement>('.headerUnitComposite')
  if (header) {
    const headerNeedsFallback = Boolean(header.querySelector('.unitTreeIcon.fallback.semantic svg.lucide-flag'))
    const selected = document.querySelector<HTMLElement>(`.unitTreeLeaf.selected[${COUNTRY_FALLBACK_ATTRIBUTE}]`)
    const label = selected?.getAttribute(COUNTRY_FALLBACK_ATTRIBUTE) || ''
    if (headerNeedsFallback && label) header.setAttribute(COUNTRY_FALLBACK_ATTRIBUTE, label)
    else header.removeAttribute(COUNTRY_FALLBACK_ATTRIBUTE)
  }
}

function scheduleCountryFallbackSync() {
  if (bridgeScheduled || typeof window === 'undefined') return
  bridgeScheduled = true
  window.requestAnimationFrame(syncCountryFallbackCards)
}

function installDomBridge() {
  if (typeof document === 'undefined' || typeof MutationObserver === 'undefined') return
  const start = () => {
    scheduleCountryFallbackSync()
    if (!document.body) return
    const observer = new MutationObserver(scheduleCountryFallbackSync)
