...
type IconEntry = { x: number; y: number; cellWidth: number; cellHeight: number; source?: string; gameFile?: string }
type IconTarget = { id: string; label: string; category: string; kind: IconKind; art_section: string }
type ArtMdRow = { section: string; Cameo: string; CameoPCX: string; AltCameoPCX: string }
type ArtMdSnapshot = { path: string; exists: boolean; rows: ArtMdRow[] }
...
function iconBackgroundStyle(kind: IconKind, entry: IconEntry, snapshot: IconLibrarySnapshot) {
  ...
  return `background-image:url(${JSON.stringify(image)});background-size:${sheetWidth}px ${sheetHeight}px;background-position:-${entry.x}px -${entry.y}px`
}
...
function renderTargets(dialog: HTMLElement, snapshot: IconLibrarySnapshot) {
  ...
  select.innerHTML = [...groups.entries()].map(([group, targets]) => `<optgroup label="${escapeHtml(group)}">${targets.map(target => {
...
function renderCards(dialog: HTMLElement, snapshot: IconLibrarySnapshot) {
  ...
  if (!host) return
  const targetMap = new Map(snapshot.targets.map(target => [`${target.kind}:${target.id.toLowerCase()}`, target]))
  const rows = [
  ...
  host.innerHTML = rows.map(({ kind, id, entry }) => {
    const target = targetMap.get(`${kind}:${id.toLowerCase()}`)
    const label = target?.label && target.label !== id ? `${target.label} · ${id}` : id
    const gameFile = entry.gameFile ? escapeHtml(entry.gameFile) : '仅编辑器资源'
    return `<div class="iconResourceCard"><div class="iconResourceCardIcon ${kind === 'country' ? 'country' : ''}" style="${iconBackgroundStyle(kind, entry, snapshot)}"></div><div class="iconResourceCardText"><b title="${escapeHtml(label)}">${escapeHtml(label)}</b><span title="${gameFile}">${gameFile}</span><span class="iconResourceBadge ${escapeHtml(entry.source || '')}">${escapeHtml(sourceLabel(entry.source))}</span></div></div>`
  }).join('')
}
...
function renderArtMd(dialog: HTMLElement, snapshot: ArtMdSnapshot, targets: IconTarget[]) {
  ...
  rows.querySelectorAll<HTMLButtonElement>('.artmdRow').forEach(button => button.addEventListener('click', () => {
    rows.querySelectorAll('.artmdRow').forEach(item => item.classList.remove('active'))
    button.classList.add('active')
    const row = snapshot.rows.find(item => item.section === button.dataset.artSection)
    if (!row) return
    const fields: Array<[string, string]> = [
      ['[data-artmd-section]', row.section], ['[data-artmd-cameo]', row.Cameo || ''],
      ['[data-artmd-pcx]', row.CameoPCX || ''], ['[data-artmd-alt-pcx]', row.AltCameoPCX || ''],
    ]
    fields.forEach(([selector, value]) => { const field = dialog.querySelector<HTMLInputElement>(selector); if (field) field.value = value })
  }))
}
...
function createDialog() {
  ...
  overlay.innerHTML = `<div class="iconResourceDialog" role="dialog" aria-modal="true" aria-label="图标资源" tabindex="-1">
    ...
    <div class="iconResourceBody">
      <section class="iconResourcePanel active" data-panel="icons">
        <div class="iconResourceToolbar"><label class="iconResourceField"><span>单位或国家</span><select data-icon-target></select></label><div class="iconResourceHint">用户图标写入独立 Tile；已有 Mod 的 Ares PCX 会自动读取。用户图标优先于 Mod 图标，原版内置图标最后兜底。</div><div class="iconResourceActions"><button type="button" class="iconResourceButton" data-icon-refresh>刷新 Mod 图标</button></div></div>
        <div class="iconResourceImport"><div class="iconResourcePreview" data-file-preview><span class="iconResourceHint">选择 PNG / JPG / BMP / WebP</span></div><div class="iconResourceImportControls">
          <input class="iconResourceHiddenInput" type="file" accept="image/png,image/jpeg,image/bmp,image/webp" data-icon-file><div class="iconResourceInline"><button type="button" class="iconResourceButton" data-icon-choose>选择图片</button><span class="iconResourceHint" data-file-name>尚未选择文件</span></div>
        ...
        </div></div><div class="iconResourceGrid" data-icon-grid></div>
      </section>
      <section class="iconResourcePanel" data-panel="artmd"><div class="artmdPath" data-artmd-path></div><div class="artmdLayout"><div class="artmdRows" data-artmd-rows></div><div class="artmdEditor">
        <h3 class="artmdEditorTitle">仅编辑图标关联</h3><label class="iconResourceField"><span>Art Section</span><input type="text" list="artmd-known-sections" data-artmd-section><datalist id="artmd-known-sections" data-artmd-sections></datalist></label>
        <label class="iconResourceField"><span>Cameo（传统 SHP 名称）</span><input type="text" data-artmd-cameo placeholder="例如 MTNKICON"></label><label class="iconResourceField"><span>CameoPCX（Ares）</span><input type="text" data-artmd-pcx placeholder="例如 mytank.pcx"></label><label class="iconResourceField"><span>AltCameoPCX（Ares 精英）</span><input type="text" data-artmd-alt-pcx placeholder="例如 mytank_elite.pcx"></label>
        <div class="iconResourceHint">这里不是完整 artmd.ini 编辑器，只管理单位图标需要的三个字段。空值保存会删除对应字段，其它 ArtMD 内容原样保留。</div><div class="iconResourceActions" style="margin-top:12px"><button type="button" class="iconResourceButton primary" data-artmd-save>保存 ArtMD 图标配置</button><button type="button" class="iconResourceButton" data-artmd-refresh>重新读取</button></div>
      </div></div></section>
    </div><div class="iconResourceStatus">就绪</div></div>`
  ...
  dialog.querySelectorAll<HTMLButtonElement>('.iconResourceTab').forEach(tab => tab.addEventListener('click', () => {
  ...
  dialog.querySelector('[data-icon-target]')?.addEventListener('change', () => { if (currentSnapshot) syncTargetState(dialog, currentSnapshot) })
  ...
  dialog.querySelector('[data-icon-refresh]')?.addEventListener('click', async () => {
    ...
    if (next) { renderSnapshot(dialog, next); status(dialog, `已刷新：识别 ${Object.keys(next.unit).length} 个单位图标、${Object.keys(next.country).length} 个国家图标。`) }
  })

  dialog.querySelector('[data-icon-import]')?.addEventListener('click', async () => {
    const target = currentSnapshot ? selectedTarget(dialog, currentSnapshot) : null
    const file = fileInput.files?.[0]
    ...
    try {
      ...
      const next = await backendCall<IconLibrarySnapshot>('import_custom_icon', {
        kind: target.kind, target_id: target.id, data_base64: await fileDataUrl(file), filename: file.name,
        sync_game: Boolean(dialog.querySelector<HTMLInputElement>('[data-sync-game]')?.checked),
        variant: dialog.querySelector<HTMLSelectElement>('[data-icon-variant]')?.value || 'cameo',
      })
      currentSnapshot = next
      cacheSnapshot(next)
      renderSnapshot(dialog, next)
      if (next.sync?.rules_dirty) markRulesDirty()
      status(dialog, next.sync?.synced ? `已应用 ${target.id}：游戏资源 ${next.sync.game_file}${next.sync.art_section ? `，Art Section [${next.sync.art_section}]` : ''}。` : `已保存 ${target.id} 的编辑器自定义图标。`)
    } catch (error) { status(dialog, `导入失败：${String(error)}`) }
  })

  dialog.querySelector('[data-icon-remove]')?.addEventListener('click', async () => {
    const target = currentSnapshot ? selectedTarget(dialog, currentSnapshot) : null
    if (!target || !window.confirm(`删除 ${target.id} 的用户自定义图标吗？\n不会自动删除游戏目录中的 PCX，也不会删除已有 ArtMD / File.Flag 关联。`)) return
    try {
      const next = await backendCall<IconLibrarySnapshot>('remove_custom_icon', { kind: target.kind, target_id: target.id })
      currentSnapshot = next
      cacheSnapshot(next)
      renderSnapshot(dialog, next)
      status(dialog, `已删除 ${target.id} 的用户图标；若 Mod 中存在可读取 PCX，会自动回退显示该 Mod 图标。`)
    } catch (error) { status(dialog, `删除失败：${String(error)}`) }
  })
  ...
  async function reloadArtMd() {
    ...
    } catch (error) { status(dialog, `读取 ArtMD 失败：${String(error)}`) }
  }
  ...
  dialog.querySelector('[data-artmd-save]')?.addEventListener('click', async () => {
    ...
    } catch (error) { status(dialog, `保存 ArtMD 失败：${String(error)}`) }
  })
...
async function openManager() {
  ...
  if (!snapshot) { status(dialog, '无法读取图标资源。'); return }
...
function start() { installAttempts = 0; retryInstall() }
