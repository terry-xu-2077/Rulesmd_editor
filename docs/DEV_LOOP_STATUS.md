...
- 首次访问新对象时，左侧选择状态立即更新，不再把整个 App 标记为 busy；异步响应完成后填充参数。
...
“添加参数”不是完整 Key 浏览器，而是针对当前对象类型的安全助手。
...
6. 添加窗口使用“分类 → 参数卡片”的浏览方式，顶部搜索只在安全结果集内搜索，不向玩家展示海量未知 Key。

这套策略优先避免错误 Key 被添加到不兼容对象后导致游戏加载异常或崩溃；后续随着规则元数据补全，可逐步扩大白名单，而不是先全量放行再补黑名单。
