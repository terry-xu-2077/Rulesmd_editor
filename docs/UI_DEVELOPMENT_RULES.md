...
**弹性容器不等于弹性控件。** 当一个横向或纵向容器存在可变剩余空间时，默认由元素之间的空白、`gap`、spacer 或 Grid/Flex 的 `1fr` 区域吸收，而不是擅自拉伸按钮、搜索框、下拉框、导航项等内部元素。

推荐布局：

```text
[固定元素] ← 弹性空白 → [固定元素] ← 弹性空白 → [固定元素]
```

例如顶部命令栏：

```text
[单位信息] ← 弹性空白 → [导航组] ← 弹性空白 → [筛选][搜索] ← 弹性空白 → [参数]
```

其中各功能组维持设计尺寸，三段空白承担容器宽度变化。窗口变宽时增加的是组间呼吸空间，窗口变窄时优先压缩这些弹性空白，而不是改变控件本体尺寸。
...
只有当某个业务元素的**语义本身就是内容区/填充区**，例如代码编辑器、表格主体、正文面板、时间轴画布等，才允许它主动占满剩余空间。对于按钮、选择器、工具栏搜索框、导航按钮等操作控件，除非设计明确要求，否则保持自身尺寸。
...
- [ ] 弹性容器优先用弹性空白分配剩余空间，没有为了填满容器而擅自拉长操作控件。
